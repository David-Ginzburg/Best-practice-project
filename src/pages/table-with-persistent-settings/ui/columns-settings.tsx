...
import { useTableColumnsSettingsStore } from '../model/store/table-columns-settings-store'
import { useTableWithPersistentSettingsColumns } from '../model/hooks/use-table-with-persistent-settings-columns'
import { getColumnLabels } from '../model/utils/column-utils'
import {
	DndContext,
	closestCenter,
	KeyboardSensor,
...
const COLUMN_LABELS = getColumnLabels()
...
export const ColumnsSettings = () => {
	...
	const { columnConfigs, setColumnOrder } = useTableWithPersistentSettingsColumns()
	const { setVisibleColumn } = useTableColumnsSettingsStore()
						...
						>
							{columnConfigs.map((config) => (
								<SortableColumnItem
									key={config.id}
									id={config.id}
									label={COLUMN_LABELS[config.id] || config.id}
