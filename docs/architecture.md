...
- **Table with Persistent Settings – Infinity scroll + grouping**: When grouping is active, automatic load-on-scroll is disabled to avoid request loops (collapsed groups leave empty space and would keep triggering load). A "Load more" button is shown instead; user triggers loading explicitly.
- **Table with Persistent Settings – Virtual list**: Optional virtual scrolling via `@tanstack/react-virtual`. When `virtualScrollConfig: { containerHeight, estimateRowHeight?, overscan? }` is passed, the table body is virtualized over `table.getRowModel().rows` (grouping and expanded state apply; only visible rows are rendered). Scroll container has fixed height; header is sticky. Compatible with pagination and infinity scroll.
